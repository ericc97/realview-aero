	...
	return (
		<nav className=" grow pt-1">
			<ul className="text-center sm:flex sm:flex-nowrap text-lg sm:justify-beginning justify-beginning  ">
				...
				<li className={`px-1 hover:bg-blue-300 sm:px-2 ${(selectionBorder.selection === 'photos') ? selectionBorder.style : 'border-2 border-transparent'}`}>
					<Link onClick={() => changeSelection('photos')} to="/Photos">Photos</Link>
				</li>
				<li className={`px-1 hover:bg-blue-300 sm:px-2 ${(selectionBorder.selection === 'videos') ? selectionBorder.style : 'border-2 border-transparent'}`}>
